...
<head>
<meta charset="UTF-8">
<title>Helping Paw - Cats</title>
</head>
<body>
	<h1>Remove Cat</h1>
	<h3> Please select the name and ID of the Cat you would like to remove.</h3>
	<table border="1">
		<form action = "RemoveCat.jsp" method = "GET">
...
<h4>
		...
		try {
			...
			{
				ResultSet rs3 = stmt2.executeQuery("select * from "+ db + "." + table + " NATURAL JOIN helpingpaw.cat WHERE animalId = " + request.getParameter("id"));
				 while (rs3.next())
			...
			int rb = stmt2.executeUpdate("DELETE FROM " + db + ".cat WHERE animalID = \'" + request.getParameter("id") + "\'");
			}
			Statement stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery("select * from "+ db + "." + table + " NATURAL JOIN helpingpaw.cat");
	...
	<h1>Cats</h1>
      ...
      <sql:query dataSource = "${snapshot}" var = "result">
         SELECT * FROM  helpingpaw.animal, helpingpaw.cat where animal.animalID = cat.animalID;
      </sql:query>
