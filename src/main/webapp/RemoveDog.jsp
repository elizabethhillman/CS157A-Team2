...
<body>
	<h1>Remove Dog</h1>
	<h3> Please select the name and ID of the Dog you would like to remove.</h3>
	<table border="1">
		<form action = "RemoveDog.jsp" method = "GET">
...
<h4>
		...
		try {
			...
			{
				ResultSet rs3 = stmt2.executeQuery("select * from "+ db + "." + table + " NATURAL JOIN helpingpaw.dog WHERE animalId = " + request.getParameter("id"));
				 while (rs3.next())
			...
			int rb = stmt2.executeUpdate("DELETE FROM " + db + ".dog WHERE animalID = \'" + request.getParameter("id") + "\'");
			}
			Statement stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery("select * from "+ db + "." + table + " NATURAL JOIN helpingpaw.dog");
